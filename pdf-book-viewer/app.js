...
const zoomLevelText = $('#zoom-level');
...
// TEST LOAD DLA BOTÓW I SZYBKIEGO TESTU
window.addEventListener('DOMContentLoaded', async () => {
    try {
        const response = await fetch('test.pdf');
        if (!response.ok) return;
        const arrayBuffer = await response.arrayBuffer();
        await loadPDF(new Uint8Array(arrayBuffer), 'test.pdf');
    } catch (err) {
        console.error('Auto-load err:', err);
    }
});
